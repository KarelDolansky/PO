...
def im2vhdl(image_path, output_path=None, mono=False, binlen=8, width=None, height=None, display=False,
            binary_packing=1, store_c=False):
    ...
    if store_c:
        write_c_file(os.path.splitext(output_path)[0], imager, binlen, channels, binary_packing, width=image.shape[1], height=image.shape[0])
    if display:
...
def write_c_file(path, data, binary_length, channels, binary_packing, width, height):
    file_c = open(path + '.c', "w")
    file_h = open(path + '.h', "w")
    image_name = os.path.basename(path)
    if file_c is None:
        print("Failed to open file" + path)
        return
    formatter = "0x{0:0" + str(int(np.ceil((channels+1) * binary_length / 4))) + "X}"
    str_list = []
    for idx in range(int(len(data) / binary_packing)):
        ...
        str_list.append(formatter.format(val))
    file_c.write(f'int {image_name}_width = {width};\n')
    file_c.write(f'int {image_name}_height = {height};\n')
    file_c.write(f'unsigned int {image_name}['+f'{len(data)}'+'] = {' + str.join(',\n', str_list) + '};')
    file_c.close()
    file_h.write('// ARGB configuration\n')
    file_h.write(f'extern int {image_name}_width;\n')
    file_h.write(f'extern int {image_name}_height;\n')
    file_h.write(f'extern unsigned int {image_name}[{len(data)}];\n')
    file_h.close()
...
def main(argv):
    ...
    parser.add_argument('-b', '--bits',
                        help='Used bits',
                        required=False)
    parser.add_argument('-c', '--clanguage',
                        action='store_true')
    ...
    im2vhdl(args.i, args.o, args.mono, int(args.bits), int(args.width) if args.width else None,
            ...
            True if args.clanguage else False)
